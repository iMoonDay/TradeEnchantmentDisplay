...
public class EnchantmentRenderer {
    ...
    private static Component generateText(List<Map.Entry<Enchantment, Integer>> entries, int drawTick) {
        ...
        if (size > 1) {
            text = new TextComponent(settings.pluralFormat.replace(FULL_NAME, fullName).replace(NAME, name).replace(LEVEL, levelText).replace(INDEX, String.valueOf(index + 1)).replace(TOTAL, String.valueOf(size)).replace(SIZE, String.valueOf(size)));
        } else {
            text = new TextComponent(settings.singularFormat.replace(FULL_NAME, fullName).replace(NAME, name).replace(LEVEL, levelText));
        }
        ...
        if (accentColor.shouldFormat(level, maxLevel)) {
            ...
            text = text.withStyle(text.getStyle().withColor(TextColor.fromRgb(settings.fontColor)));
        }
        return text;
    }
    ...
    public static boolean shouldPass(ItemStack stack, boolean onlyEnchantedBooks) {
        return stack.getItem() != Items.ENCHANTED_BOOK && (onlyEnchantedBooks || !stack.isEnchanted());
    }
    ...
    public static void renderUnderNameTag(Entity entity, MerchantOfferInfo info, PoseStack poseStack, MultiBufferSource buffer, EntityRenderDispatcher dispatcher, Font font, int packedLight) {
        ...
        if (!settings.enabled) return;
        List<MerchantOffer> offers = info.getOffers(offer -> offer.getResult().getItem() == Items.ENCHANTED_BOOK && checkBlackList(offer));
        ...
        for (Map.Entry<Enchantment, Integer> entry : entries) {
            ...
            MutableComponent name = enchantment.getFullname(level).copy().setStyle(Style.EMPTY.withColor(TextColor.fromRgb(nameColor)));
    ...
    public static boolean isStandardEnchantedBookTrade(MerchantOffer offer) {
        return offer.getCostA().getItem() == Items.EMERALD && offer.getCostB().getItem() == Items.BOOK && offer.getResult().getItem() == Items.ENCHANTED_BOOK;
    }
