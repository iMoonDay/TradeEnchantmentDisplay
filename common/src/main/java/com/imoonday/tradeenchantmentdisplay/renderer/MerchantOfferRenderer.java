...
public class MerchantOfferRenderer {
    ...
    public MerchantOfferRenderer(Minecraft mc, PoseStack poseStack, ModConfig.Hud settings) {
        this(mc, poseStack, mc.font);
    ...
    public static boolean shouldRenderCostB(ItemStack costA, ItemStack costB, ItemStack result) {
        return !(costA.getItem() == Items.EMERALD && costB.getItem() == Items.BOOK && result.getItem() == Items.ENCHANTED_BOOK || costB.isEmpty());
    }
    ...
    public void render(MerchantOffer offer, int x, int y) {
        ...
        int itemY = y + 21;
        if (dividerColor != 0) {
            Screen.fill(poseStack, x, itemY - 3, x + 16, itemY - 2, dividerColor);
        }
    ...
    public void render(List<MerchantOffer> offers, int x, int y, Predicate<MerchantOffer> filter) {
        ...
        if (offers.isEmpty()) {
            if (tipForNoEnchantment && MerchantOfferHandler.getValidMerchant(mc) != null) {
                ...
                Screen.fill(poseStack, x - paddingX, y - paddingY, x + font.width(text) + paddingX, y + font.lineHeight + paddingY - 1, bgColor);
                Screen.drawString(poseStack, font, text, x, y, fontColor);
            }
        } else {
            final int startY = y;
            int maxX = 0;
            ...
            for (MerchantOffer offer : offers) {
                ...
                renderBackground(x, y, (int) dimensions.x, (int) dimensions.y);
                render(offer, x, y);
                y += (int) (dimensions.y + paddingY * 3);
                maxX = (int) Math.max(maxX, x + dimensions.x);
            }
        }
        poseStack.popPose();
    }
    ...
    public void renderItem(ItemStack result, int x, int y) {
        ...
        itemRenderer.renderAndDecorateFakeItem(result, x, y);
        itemRenderer.renderGuiItemDecorations(font, result, x, y);
    }
    ...
    public MutableComponent getFormattedName(Enchantment enchantment, int level) {
        MutableComponent name = enchantment.getFullname(level).copy().setStyle(Style.EMPTY.withColor(TextColor.fromRgb(fontColor)));
